...
(function () {
  ...
  var DEFAULT = {pal:'elevare', type:'elevare', weight:'bold', letter:'normal', line:'normal', italic:true, radius:'round', device:'live', skills:'1', home:'1', pkg:'1', form:'1'};
  ...
  function apply(){
    ...
    // page colour comes from html{background:var(--bg)}; keep body transparent so
    // page-level background images (e.g. offers) remain visible.
    ...
    r.setAttribute('data-skills', state.skills);
    r.setAttribute('data-home', state.home);
    r.setAttribute('data-pkg', state.pkg);
    r.setAttribute('data-form', state.form);
    save(state);
    syncUI();
  }
  ...
  var IC = {
    ...
    layout: svg('<rect x="4" y="4" width="7" height="7" rx="1"/><rect x="13" y="4" width="7" height="7" rx="1"/><rect x="4" y="13" width="7" height="7" rx="1"/><rect x="13" y="13" width="7" height="7" rx="1"/>'),
    home: svg('<path d="M4 11l8-6 8 6"/><path d="M6 10v9h12v-9"/>'),
    heroClassic: svg('<path d="M5 8h14M7 12h10M9 16h6"/>'),
    heroBento: svg('<rect x="4" y="5" width="9" height="14" rx="1"/><rect x="15" y="5" width="5" height="6" rx="1"/><rect x="15" y="13" width="5" height="6" rx="1"/>'),
    heroPanel: svg('<rect x="3" y="5" width="18" height="11" rx="2.5"/><rect x="7" y="17" width="4" height="3" rx="1"/><rect x="13" y="17" width="4" height="3" rx="1"/>'),
    rows: svg('<rect x="4" y="5" width="16" height="4" rx="1"/><rect x="4" y="11" width="16" height="4" rx="1"/><rect x="4" y="17" width="16" height="3" rx="1"/>'),
    spotlight: svg('<rect x="4" y="8" width="4" height="8" rx="1"/><rect x="10" y="4" width="4" height="16" rx="1"/><rect x="16" y="8" width="4" height="8" rx="1"/>'),
    split: svg('<rect x="3" y="5" width="8" height="14" rx="1.5"/><rect x="13" y="5" width="8" height="14" rx="1.5"/>'),
    stacked: svg('<rect x="4" y="4" width="16" height="6" rx="1.5"/><rect x="4" y="12" width="16" height="8" rx="1.5"/>'),
    open: svg('<rect x="5" y="4" width="14" height="16" rx="2"/><path d="M9 10h6M9 14h6"/>'),
    night: svg('<path d="M20 14.2A8 8 0 119.8 4 6.2 6.2 0 0020 14.2Z"/>'),
    zigzag: svg('<rect x="3" y="6" width="11" height="4" rx="1"/><rect x="10" y="14" width="11" height="4" rx="1"/>'),
    sidebar: svg('<rect x="3" y="5" width="11" height="14" rx="1.5"/><rect x="16" y="5" width="5" height="14" rx="1.5"/>'),
    editorial: svg('<path d="M5 5h14M5 9h8M5 13h14M5 17h8"/>'),
    list: svg('<path d="M4 6h16M4 12h16M4 18h16"/>'),
    centered: svg('<rect x="6" y="4" width="12" height="16" rx="2"/><path d="M9 9h6M9 13h6"/>'),
    flow: svg('<path d="M4 8h5M11 8h4M17 8h3M4 13h3M9 13h6M17 13h3M4 18h4M10 18h5"/>'),
    splitL: svg('<rect x="3" y="5" width="8" height="14" rx="1.5"/><path d="M15 8h4M15 12h4M15 16h4"/>'),
    bold: svg('<path d="M7 5h5.5a3.2 3.2 0 010 6.4H7zM7 11.4h6.2a3.3 3.3 0 010 6.6H7z"/>'),
    outline: svg('<rect x="4" y="5" width="16" height="14" rx="2.5"/>'),
    showcase: svg('<rect x="4" y="4" width="16" height="6" rx="1.5"/><rect x="4" y="12" width="7" height="7" rx="1.5"/><rect x="13" y="12" width="7" height="7" rx="1.5"/>'),
    heroband: svg('<rect x="3" y="4" width="18" height="7" rx="2"/><rect x="6" y="13" width="12" height="6" rx="1.5"/>'),
    dark: svg('<path d="M20 14.2A8 8 0 119.8 4 6.2 6.2 0 0020 14.2Z"/>'),
    band: svg('<rect x="3" y="6" width="18" height="5" rx="1.5"/><rect x="3" y="13" width="18" height="5" rx="1.5"/>'),
    indexcols: svg('<path d="M4 6h7M4 11h7M4 16h7M13 6h7M13 11h7M13 16h7"/>'),
  ...
  function build(){
    ...
    panel.innerHTML =
      ...
      '<div class="ds-body">'+
        ...
        ((/(^|\/)index\.html(?:$|[?#])/.test(location.pathname) || location.pathname === '/' || /\/$/.test(location.pathname)) ?
          '<div class="ds-sec-label">'+IC.home+' Home page &middot; Hero style</div>'+
          '<div class="ds-seg" data-group="home" style="grid-auto-flow:row;grid-template-columns:1fr 1fr 1fr;">'+
            '<button data-v="1">'+IC.heroClassic+'Classic</button><button data-v="2">'+IC.heroBento+'Bento</button><button data-v="3">'+IC.heroPanel+'Panel</button><button data-v="4">'+IC.editorial+'Editorial</button></div>'
          : '')+

        ((/(^|\/)offers\.html(?:$|[?#])/.test(location.pathname) || /\/offers(?:$|[?#])/.test(location.pathname)) ?
          '<div class="ds-sec-label">'+IC.layout+' Offers page &middot; Packages layout</div>'+
          '<div class="ds-seg" data-group="pkg" style="grid-auto-flow:row;grid-template-columns:1fr 1fr 1fr;">'+
            '<button data-v="1">'+IC.heroBento+'Cards</button><button data-v="2">'+IC.rows+'Rows</button><button data-v="3">'+IC.spotlight+'Spotlight</button><button data-v="4">'+IC.zigzag+'Zigzag</button><button data-v="5">'+IC.list+'List</button><button data-v="6">'+IC.outline+'Outline</button><button data-v="7">'+IC.showcase+'Showcase</button></div>'
          : '')+

        ((/(^|\/)contact\.html(?:$|[?#])/.test(location.pathname) || /\/contact(?:$|[?#])/.test(location.pathname)) ?
          '<div class="ds-sec-label">'+IC.layout+' Contact page &middot; Form layout</div>'+
          '<div class="ds-seg" data-group="form" style="grid-auto-flow:row;grid-template-columns:1fr 1fr 1fr;">'+
            '<button data-v="1">'+IC.split+'Split</button><button data-v="2">'+IC.stacked+'Stacked</button><button data-v="3">'+IC.open+'Open</button><button data-v="4">'+IC.sidebar+'Sidebar</button><button data-v="5">'+IC.centered+'Centered</button><button data-v="6">'+IC.heroband+'Hero</button><button data-v="7">'+IC.dark+'Dark</button></div>'
          : '')+

        (/hire\.html(?:$|[?#])/.test(location.pathname + location.search + location.hash) || /\/hire(?:$|[?#])/.test(location.pathname) ?
          '<div class="ds-sec-label">'+IC.layout+' Hire page &middot; Top Skills layout</div>'+
          '<div class="ds-seg" data-group="skills" style="grid-auto-flow:row;grid-template-columns:1fr 1fr 1fr;">'+
            '<button data-v="1">'+IC.layout+'Columns</button><button data-v="2">'+IC.rows+'Tags</button><button data-v="3">'+IC.heroBento+'Cards</button><button data-v="4">'+IC.layout+'Grid</button><button data-v="5">'+IC.flow+'Flow</button><button data-v="6">'+IC.band+'Bands</button><button data-v="7">'+IC.indexcols+'Index</button></div>'
          : '')+
